"""Generate submission.csv using the trained Transformer agent, following
the same blueprint as generate_submission_bilstm.py: play the actual
adaptive game against each test.txt word, terminating the instant a 6th
wrong guess lands or the word is solved, join the guesses made into one
flat string. Run in the Kaggle notebook right after training, no need to
save/reload weights first.
"""
import csv
import os
import sys
...
from transformer_agent import TransformerAgent, DEFAULT_MODEL_PATH
...
def main(limit=None):
    ...
    agent = TransformerAgent(model_path=DEFAULT_MODEL_PATH, device=device)
