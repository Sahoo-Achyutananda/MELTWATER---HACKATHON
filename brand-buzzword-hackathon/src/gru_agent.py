"""Wraps the trained BiGRU masked-letter predictor into the standard
guess(pattern, guessed_letters) interface -- identical aggregation logic
to bilstm_agent.py: sum per-position softmax probability across every
blank, zero out already-guessed letters, return the argmax.
"""
...
class GRUAgent:
    def __init__(self, model_path: str = DEFAULT_MODEL_PATH, device: str = "cpu"):
        ...
        self.model = BiGRUMasker().to(self.device)
