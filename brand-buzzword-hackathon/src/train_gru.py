"""Train the BiGRU masked-letter predictor on train.txt. Identical
methodology to train_bilstm.py (approach/bilstm branch) -- batches formed
within same-length buckets (no padding needed), random per-batch mask
fraction in [0.15, 0.85] so the model learns from both early- and
late-game board states.
"""
...
from gru_model import BiGRUMasker, LETTER_IDX, MASK_TOKEN, LETTER_TOKEN_OFFSET
