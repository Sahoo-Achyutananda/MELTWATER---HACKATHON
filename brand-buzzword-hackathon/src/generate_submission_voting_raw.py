"""Generate submission.csv using the voting ensemble agent -- same
blueprint as the other generate_submission_*.py scripts: play the actual
...
def main(limit=None):
    ...
    print(f"agent built on {len(train_words)} words in {time.time()-t0:.1f}s, families={agent.families}")

    rows, wins, total_wrong = [], 0, 0
    t0 = time.time()
    ...
    n = len(test_words)
    print(f"FINAL win_rate={wins/n:.4f} avg_wrong={total_wrong/n:.2f} n={n} time={time.time()-t0:.0f}s")
