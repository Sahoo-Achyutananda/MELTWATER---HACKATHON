"""Generate submission.csv using the combined agent (candidate-filtering +
n-gram + BiLSTM + vowel guard) -- same blueprint as the other
generate_submission_*.py scripts: play the actual adaptive game against
each test.txt word, terminating the instant a 6th wrong guess lands or
the word is solved, join the guesses made into one flat string. Run in
the Kaggle notebook right after training, no need to save/reload weights
first.
"""
import csv
import os
